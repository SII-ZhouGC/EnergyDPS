...
def convert_chameleon_weights(chameleon, cfg: HookedTransformerConfig):
    ...
    # Some models with the Chameleon architecture use Grouped Query Attention, and so for these we need to modify
    ...
    # chameleon has no biases anywhere and deals with everything else roughly like
    ...
    for l in range(cfg.n_layers):
        ...
        state_dict[f"blocks.{l}.attn.ln_q.w"] = chameleon.model.layers[l].self_attn.q_norm.weight
        state_dict[f"blocks.{l}.attn.ln_q.b"] = chameleon.model.layers[l].self_attn.q_norm.bias
        state_dict[f"blocks.{l}.attn.ln_k.w"] = chameleon.model.layers[l].self_attn.k_norm.weight
        state_dict[f"blocks.{l}.attn.ln_k.b"] = chameleon.model.layers[l].self_attn.k_norm.bias
