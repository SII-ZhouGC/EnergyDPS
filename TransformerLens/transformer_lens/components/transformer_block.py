...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.cfg.use_post_layer_norm:
            # Post-layer-norm: apply attention first, then normalize (used in Olmo2)
            attn_out = self.attn(
            ...
            # Pre-layer-norm: normalize first, then apply attention (standard)
            attn_out = (
        ...
        if not self.cfg.attn_only and not self.cfg.parallel_attn_mlp:
            ...
            if self.cfg.use_post_layer_norm:
                # Post-layer-norm: apply MLP first, then normalize (used in Olmo2)
                mlp_out = self.apply_mlp(mlp_in)
                mlp_out = self.ln2(mlp_out)
            else:
                # Pre-layer-norm: normalize first, then apply MLP (standard)
